...
if top_frequency>sample_rate//2:
	...
	with Image.open(input_file) as image:
		...
		samples_total=0
		while image_x<image.width:
			for sample in range(0,sample_size):
				frequency=0
				byte=0
				print(str(int(samples_total/(sample_size*image.width)*100)) + "% done, X: " + str(image_x+1) + "/" + str(image.width) + ", Sample: " + str(sample+1) + "/" + str(sample_size) + "       ",end="\r")
